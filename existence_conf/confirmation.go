...
func (c *ExistenceConf) Conf(data *dpfm_api_input_reader.SDC, ssdc *dpfm_api_output_formatter.SDC, l *logger.Logger) (allExist bool, errs []error) {
	...
	wg.Add(6)
	go c.productTypeExistenceConf(data, &existenceMap, &resMsg, &errs, mtx, &wg, l)
	go c.baseUnitExistenceConf(data, &existenceMap, &resMsg, &errs, mtx, &wg, l)
	go c.productGroupExistenceConf(data, &existenceMap, &resMsg, &errs, mtx, &wg, l)
	go c.weightUnitExistenceConf(data, &existenceMap, &resMsg, &errs, mtx, &wg, l)
	go c.countryOfOriginExistenceConf(data, &existenceMap, &resMsg, &errs, mtx, &wg, l)
	go c.countryOfOriginLanguageExistenceConf(data, &existenceMap, &resMsg, &errs, mtx, &wg, l)
