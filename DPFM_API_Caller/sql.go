...
func (c *DPFMAPICaller) createSqlProcess(
	...
	for _, fn := range accepter {
		switch fn {
		case "General":
			general = c.generalCreateSql(nil, mtx, input, output, subfuncSDC, errs, log)
		default:
...
func (c *DPFMAPICaller) generalCreateSql(
	...
	// data_platform_orders_general_dataの更新
	generalData := subfuncSDC.Message.General
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": generalData, "function": "OrdersGeneral", "runtime_session_id": sessionID})
	if err != nil {
	...
	data := dpfm_api_output_formatter.ConvertToGeneral(subfuncSDC)
...
func (c *DPFMAPICaller) generalUpdateSql(
	...
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": req, "function": "OrdersGeneral", "runtime_session_id": 123})
	if err != nil {
		err = xerrors.Errorf("rmq error: %w", err)
		return nil
	}
	...
	data := dpfm_api_output_formatter.ConvertToGeneral(req)
